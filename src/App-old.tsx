...
function App() {
  ...
  useEffect(() => {
        ...
        if (webviewAny.getWebContentsId && window.electronAPI?.playwright) {
          ...
          if (webContentsId) {
            const result = await window.electronAPI.playwright.connectWebview(webContentsId, {
              width: Math.floor(webview.getBoundingClientRect().width),
              height: Math.floor(webview.getBoundingClientRect().height)
            });
            if (result.success) {
    ...
    // 창 크기 변경 시 Playwright 뷰포트 업데이트
    const handleResize = () => {
      if (webviewRef.current && window.electronAPI?.playwright?.setViewport) {
        const rect = webviewRef.current.getBoundingClientRect();
        window.electronAPI.playwright.setViewport(Math.floor(rect.width), Math.floor(rect.height));
      }
    };
